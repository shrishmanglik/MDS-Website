interface SectionDividerProps {
  className?: string
  variant?: 'gold' | 'blue' | 'subtle' | 'glow' | 'pulse'
}
...
const GRADIENTS = {
  ...
  glow: 'from-transparent via-[rgba(41,98,255,0.4)] to-transparent',
  pulse: 'from-transparent via-[rgba(212,175,55,0.3)] to-transparent',
} as const
...
export function SectionDivider({ className = '', variant = 'gold' }: SectionDividerProps) {
  return (
    <div className={`relative h-px w-full max-w-content mx-auto ${className}`}>
      <div className={`absolute inset-0 bg-gradient-to-r ${GRADIENTS[variant]} ${variant === 'glow' ? 'scale-x-125 blur-[0.5px]' : ''} ${variant === 'pulse' ? 'animate-divider-pulse' : ''}`} />
    </div>
  )
}
