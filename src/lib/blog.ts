...
export const blogPosts: BlogPost[] = [
  {
    ...
    content: `
      ...
      <p>Our product suite isn't a random collection of tools. Every product solves a specific, painful problem that I've personally experienced or watched others struggle with. Pathway AI Studio delivers TEF Canada preparation for French-speaking Express Entry applicants who are tired of cloud AI tools hallucinating their grammar scores. ATLAS encodes the India-Canada DTAA treaty as pure computation so NRI tax planning stops depending on chatbots. Astro AI Studio makes Swiss-Ephemeris-grade Vedic astrology available offline to professional astrologers. PrepAI Studio brings exam-accurate practice papers to JEE/NEET students without per-question AI costs. Each tool is designed around a single philosophy: <strong>solve a real problem in a specific domain better than generic AI tools ever could</strong>. Our AI products are purpose-built for their industries, with deep domain expertise baked into every interaction.</p>
  ...
  },
  {
    ...
    content: `
      ...
      <p>One of the biggest advantages of being a <a href="/about" class="text-accent-purple hover:underline">solo builder</a> is that architecture decisions happen in seconds, not weeks. There are no meetings about whether to use REST or GraphQL. No debates about monorepo vs. polyrepo. No committees evaluating state management libraries. I make a decision, commit to it, and move on. For <a href="/products" class="text-accent-purple hover:underline">our product suite</a>, those decisions are: <strong>Next.js 15 with App Router</strong> for everything web-facing. <strong>TypeScript in strict mode</strong> because life is too short for runtime type errors. <strong>Tailwind CSS</strong> because utility-first is faster than writing custom CSS. <strong>On-device AI models via WebAssembly and WebGPU</strong> for the offline tools. <strong>Edge functions</strong> for anything that needs a server. These aren't necessarily the "best" choices in some abstract sense. They're the best choices for a solo founder who needs to ship multiple products rapidly while maintaining quality that users will actually pay for.</p>

      <h2>The Product Factory Model</h2>
      <p>I think of this as a product factory, not a startup. A startup builds one thing and hopes it works. A product factory builds a pipeline — shared infrastructure, shared design language, shared deployment, shared billing — and then stamps out products on top of that foundation. Every product shares the same authentication system, the same UI component library, the same deployment pipeline, and the same landing page structure. When I build Pathway AI Studio, the work I do on the shared scoring-engine patterns benefits ATLAS. When I optimize the deterministic architecture for Astro AI Studio, that optimization carries over to PrepAI Studio and every other product. <strong>The next product takes a fraction of the time the first one did</strong>, not because I'm cutting corners, but because the foundation is already rock-solid.</p>
  ...
  },
  {
    ...
    content: `
      ...
      <p>Half of <a href="/products" class="text-accent-purple hover:underline">our AI products</a> run entirely on your device. Not "processes locally and syncs to the cloud." Not "stores data locally but sends it for processing." <strong>Entirely on your device.</strong> Our offline-capable tools use on-device language models that run in your browser via WebAssembly and WebGPU. The model weights are downloaded once and cached locally. When you interact with the tool, the computation happens on your CPU and GPU. The response is generated without a single network request. Your data never exists anywhere except your own hardware. Our local AI processing handles speech-to-text using Whisper models running locally — your audio never leaves your machine. On-device AI powers image and design tasks without uploading your creative work to anyone else's server. This isn't a marketing gimmick. Open your browser's developer tools, go to the Network tab, and <a href="/free-audit" class="text-accent-purple hover:underline">verify it yourself</a>. Zero outbound requests during processing.</p>
